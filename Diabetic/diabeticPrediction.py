...
def predictNextYearDiabeticClass(data):
    ...
    try:
        ...
        return ((str(e)) + ' Required Fields: ' + ', '.join(
            str(x)
            for x in [
                'L104600', 'L103000', 'S000300', 'L101700', 'L100700',
                'FIELD_33', 'FIELD_38', 'FIELD_40', 'FIELD_31', 'SEX', 'AGE'
            ]))
...
def predictDiabeticNextYearValue(data):
    ...
    try:
        ...
        temppredictedvalue = pd.DataFrame( predictedNextYearValues.copy(deep=True)).reset_index(drop=True)
        temppredictedvalue.columns = ['L100800_y', 'L104600_y', 'L103000_y', 'S000300_y', 'L101700_y', 'L100700_y', 'SEX_y', 'AGE_y',
                                      'FIELD_33_y', 'FIELD_38_y', 'FIELD_40_y', 'FIELD_31_y']

        tempthisyeardata = pd.DataFrame(data[["L100800", "L104600", "L103000", "S000300", "L101700", "L100700", "FIELD_33",
                                 "FIELD_38", "FIELD_40", "FIELD_31", "SEX", "AGE"]]).reset_index(drop=True)
        
        testdata = pd.concat([temppredictedvalue, tempthisyeardata], axis=1)

        predictedNextYearValues.columns=[
                'fasting glucose', 'HBA1C', 'Triglycerides', 'BMI', 'r-GTP gamma',
            'Uric Acid', 'Smoking', 'Drinking', 'physical_activity', 'Family_history', 'SEX', 'AGE'
        ]
...
def predictDiabeticClassForAdustedValues(data, thisyearvalue):
    ...
    try:
        ...
        data.columns = ['L100800_y', 'L104600_y', 'L103000_y', 'S000300_y', 'L101700_y', 'L100700_y',
                        'FIELD_33_y', 'FIELD_38_y', 'FIELD_40_y', 'FIELD_31_y', 'SEX_y', 'AGE_y']
        # rename the columns to match the orginal model features name
        thisyearvalue = thisyearvalue[[
            'L100800', 'L104600', 'L103000', 'S000300', 'L101700', 'L100700', 'FIELD_33',
